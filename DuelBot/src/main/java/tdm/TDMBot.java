...
/**
 * TDM BOT TEMPLATE CLASS Version: 0.0.1
 */
@AgentScoped
public class TDMBot extends UT2004BotTCController<UT2004Bot> {
    ...
    /**
     * TRUE => rebinds NAVMESH+NAVIGATION GRAPH; useful when you add new map
     * tweak into {@link MapTweaks}.
     */
    ...
    /**
     * 0-based; note that during the tournament all your bots will have
     * botInstance == 0!
     */
    ...
    /**
     * This is a place where you should use map tweaks, i.e., patch original
     * Navigation Graph that comes from UT2004.
     */
    @Override
    ...
    /**
     * Define your weapon preferences here (if you are going to use
     * weaponPrefs).
     */
    ...
    /**
     * Bot has been initialized inside GameBots2004 (Unreal Tournament 2004) and
     * is about to enter the play (it does not have the body materialized yet).
     *
     * @param gameInfo
     * @param currentConfig
     * @param init
    ...
    // ==========================
    /**
     * {@link PlayerDamaged} listener that senses that "some other bot was
     * hurt".
     *
     * @param event
     */
    ...
    /**
     * {@link HearNoise} listener that senses that "some noise was heard by the
     * bot".
     *
     * @param event
     */
    ...
    /**
     * {@link ItemPickedUp} listener that senses that "your bot has picked up
     * some item".
     *
     * See sources for {@link ItemType} for details about item types /
     * categories / groups.
     *
     * @param event
     */
    ...
    /*
     Ally started fighting some enemy
     */
    @EventListener(eventClass = TCFightingEnemy.class)
    public void tcAllyEngagedEnemy(TCFightingEnemy event) {
        log.warning("BOT " + event.getWho() + " ENGAGED " + event.getWhat());
        Player player = players.getPlayer(event.getWhat());
        behaviorManager.GetResources().otherEnemy = player;
    ...
    /**
     * {@link IncomingProjectile} listener that senses that "some projectile has
     * appeared OR moved OR disappeared".
     *
     * @param event
     */
    ...
    /**
     * {@link Player} listener that senses that "some other bot has appeared OR
    ...
    /**
     * {@link Self} object has been updated. This update is received about every
     * 50ms. You can use this update to fine-time some of your behavior like
     * "weapon switching". I.e. SELF is updated every 50ms while LOGIC is
     * invoked every 250ms.
     *
     * Note that during "SELF UPDATE" only information about your bot
     * location/rotation ({@link Self}) is updated. All other visibilities
     * remains the same as during last {@link #logic()}.
     *
     * Note that new {@link NavMeshNavigation} is using SELF UPDATES to
     * fine-control the bot's navigation.
     *
     * @param event
     */
     ...
     *
     * This is a typical place from where you start coding your bot. Even though
     * bot can be completely EVENT-DRIVEN, the reactive aproach via "ticking"
     * logic() method is more simple / straight-forward.
     */
    @Override
    public void logic() {
    ...
    // ===========
    /**
     * It is good to log that the logic has started so you can then
     * retrospectively check the batches.
     */
    ...
    /**
     * It is good in-general to periodically log anything that relates to your's
     * {@link TDMBot#logic()} decision making.
     *
     * You might consider exporting these values to some custom Swing window
     * (you crete for yourself) that will be more readable.
     */
    public void logMind() {
    ...
    // ======================================
    /**
     * Returns path-nearest {@link NavPoint} that is covered from 'enemy'. Uses
     * {@link UT2004BotModuleController#getVisibility()}.
     *
     * @param enemy
     * @return
     */
    ...
    /**
     * Returns whether you can actually pick this 'item', based on "isSpawned"
     * and "isPickable" in your current state and knowledge.
     */
     ...
     *
     * It is not sensible to perform more than 1000 raycasts per logic() per
     * bot.
     *
     * NOTE THAT IN ORDER TO USE THIS, you have to rename "map_" folder into
     * "map" ... so it would load the level geometry. Note that loading a level
     * geometry up takes quite a lot of time (>60MB large BSP tree...).
     *
     * @param from
     * @param to
     * @return
    ...
    /**
     * Performs a client-side raycast against NavMesh in 'direction'. Returns
     * distance of the edge in given 'direction' sending the ray 'from'.
     *
     * @param from
     * @param direction
     * @return
